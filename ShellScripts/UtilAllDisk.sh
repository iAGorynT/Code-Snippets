...
# Header
clear
format_printf "Cleanup All Disks..." "yellow" "bold"
printf "\n"

# Display packages that will be updated
for i in {1..${#updaters[@]}}; do
    update_printf "Will cleanup: ${package_names[$i]}"
done
printf "\n"

# Ask if user wants to run all package updates
if get_yes_no "$(format_printf "Do you want to run All Disk Cleanup?" none "rocket")"; then
    ...
    error_printf "Cleanup All Disks cancelled by user."
fi
